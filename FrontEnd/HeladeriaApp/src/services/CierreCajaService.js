...
const CierreCajaService = {
  ...
  imprimirResumen: async (id) => {
    try {
      const resultado = await imprimirResumenCierre(id);
      console.log(`Resumen del cierre con ID: ${id} enviado a impresión.`);
      return resultado;
    } catch (error) {
      console.error(
        `Error al imprimir resumen del cierre con ID: ${id}`,
        error
      );
      throw error; // Re-lanzar el error para que pueda ser manejado en el componente
    }
