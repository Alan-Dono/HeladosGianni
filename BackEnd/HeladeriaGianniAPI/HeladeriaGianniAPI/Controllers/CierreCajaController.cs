...
namespace HeladeriaGianniAPI.Controllers
{
    ...
    public class CierreCajaController : ControllerBase
    {
        ...
        public async Task<IActionResult> ObtenerResumenProductosDelCierre(int id)
        {
            try
            {
                var cierre = await _cierreCajaService.ObtenerCierreConResumen(id);

                if (cierre == null)
                    return NotFound($"No se encontró un cierre de caja con ID: {id}");

                var resumen = cierre.Ventas
        ...
        public async Task<IActionResult> ImprimirResumenCierre(int id)
        {
            try
            {
                await _cierreCajaService.ImprimirResumenCierre(id);
                return Ok("Resumen de cierre enviado a la impresora.");
            }
